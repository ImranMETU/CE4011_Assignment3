...
The Q1 implementation (symmetric sparse matrix, vector, and conjugate gradient solver) was reused because:
\begin{itemize}
...
% ------------------------------------------------------------------
\subsection*{Semantic Discrepancy Notes}
% ------------------------------------------------------------------

Two naming inconsistencies exist between the module-level legacy functions
in \texttt{frame\_element.py} and the object-oriented interface:

\begin{itemize}
  \item \texttt{compute\_rotation\_matrix()},
  \texttt{compute\_global\_stiffness()}, \texttt{matrix\_multiply()},
  \texttt{matrix\_transpose()}, and \texttt{print\_matrix()} are defined
  at module level but are never called by \texttt{FrameElement} or
  \texttt{Element}.
  The active implementation resides in
  \texttt{Element.transformation\_matrix()} and
  \texttt{Element.global\_stiffness()}.
  These legacy functions are dead code and represent a semantic
  discrepancy between what the module exposes and what the class
  hierarchy actually uses.
  They should be removed before final submission to prevent confusion.

  \item The parameter named \texttt{R} in the legacy
  \texttt{compute\_rotation\_matrix()} function is mathematically the
  transformation matrix \texttt{T} (global-to-local), not a rotation
  matrix in the conventional sense.
  The object-oriented code avoids this ambiguity by naming the method
  \texttt{transformation\_matrix()} and documenting its direction
  explicitly.
\end{itemize}
...
\begin{itemize}
    \item Frame elements include axial, shear, and bending behavior.
