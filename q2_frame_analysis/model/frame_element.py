...
def compute_rotation_matrix(angle):
    """Compute 6x6 rotation matrix for 2D frame element."""
    c = math.cos(angle)
    s = math.sin(angle)
...
def matrix_transpose(M, size):
    """Compute transpose of square matrix."""
    M_T = [[M[j][i] for j in range(size)] for i in range(size)]
    return M_T


def compute_global_stiffness(k_prime, angle):
    """Compute global stiffness matrix k = R^T * k' * R."""
    R = compute_rotation_matrix(angle)
    R_T = matrix_transpose(R, 6)
    temp = matrix_multiply(R_T, k_prime, 6)
    k = matrix_multiply(temp, R, 6)
    return k
